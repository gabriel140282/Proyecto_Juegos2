...
public class HanoiView extends JPanel implements MouseListener, MouseMotionListener {
    ...
    private JTextArea logArea; // ✅ Log lateral
    ...
    public void logMovimiento(String texto) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        int w = getWidth() - 200; // ajustar ancho por log lateral
        int h = getHeight();
        int baseY = h - 40;
        int spacing = w / 4;
        ...
        if (dragging) {
            ...
            g2.setColor(c);
            g2.fillRoundRect(dx, dyPos, wDisk, dy, 8, 8);
            g2.setColor(Color.BLACK);
            g2.drawRoundRect(dx, dyPos, wDisk, dy, 8, 8);
        }
    }

    private int pegAt(int x) {
        int spacing = (getWidth() - 200) / 4;
        for (int i = 0; i < 3; i++) {
